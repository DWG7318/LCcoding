...
- The shipped desktop application is Tauri 2 with a Rust projection core and a bundled Vanilla HTML/CSS/TypeScript frontend built by Vite. It ships no Python subprocess, second runtime, frontend framework, or remote web application.
...
Exactly six `Open` actions are available: Proposal Readiness, Canonical Candidate, Calabash, Simulation World, Workflow, and UI Baseline. `Open` replaces the body inside the same window with a fixed, sanitized report; `Back` restores the prior phase view and scroll/fold state.
...
- Calabash may show its validated current version when a complete Canonical Manifest supplies it. Under the two-input contract, Simulation, Workflow, and UI have no version source and always show Not recorded/未记录; implementation must not read another file or add a status field to invent one.
...
| `simulation` | `null` | `row.status` | `status.simulation` | `view_state` |
| `simulation` | `null` | `row.current_phase` | `status.current_phase` | `phase` |
| `workflow` | `null` | `row.status` | `status.workflow` | `view_state` |
| `workflow` | `null` | `row.current_phase` | `status.current_phase` | `phase` |
| `ui` | `null` | `row.status` | `status.ui` | `view_state` |
| `ui` | `null` | `row.current_phase` | `status.current_phase` | `phase` |

Each report's `state` equals its associated main-step state: Proposal, Project Initialization, Calabash Draft, Simulation, Workflow, or UI respectively. `StepView.report` is non-null only for those six steps and equals the corresponding report ID; all other steps use `null`. Candidate/Manifest invalidity is a whole-record error, never a report-only warning.
...
│   ├── render/report-view.ts         # six fixed protected reports
...
For record/schema/truth errors, `get_snapshot()` succeeds at IPC level and returns the exact error Snapshot: `health="error"`, project `Unnamed project`, `current_phase="UNKNOWN"`, all four phases and every fixed step `error`, all six reports `error`, versions `null`, state rows `error`, phase rows `UNKNOWN`, and lock/record rows `UNKNOWN`. Startup argument failure occurs before a window exists and emits only fixed code `BI_STARTUP_INPUT_INVALID` with process exit `2`. No raw error is serialized or printed. Topmost failures use internal code `BI_PIN_UNAVAILABLE`; this diagnostic code is never rendered as user-facing text.
...
## 5. Input safety and strict schemas
...
- Configure an explicit CSP no broader than: `default-src 'self'; script-src 'self'; style-src 'self'; img-src 'self'; font-src 'self'; connect-src ipc: http://ipc.localhost; object-src 'none'; base-uri 'none'; frame-src 'none'; form-action 'none'`. Keep Tauri CSP modification enabled so it can apply build-time hashes/nonces.
- `build.rs` declares exactly `get_snapshot`, `set_pinned`, and `is_pinned` through `AppManifest::commands`; this generates only their allow/deny ACL entries and does not register handlers. `capabilities/main.json` has `windows:["main"]`, no `remote`, and permissions exactly `allow-get-snapshot`, `allow-set-pinned`, and `allow-is-pinned`. It does not use `core:default`, `core:window:default`, or any plugin default. `main.rs` separately registers exactly the same three functions with `invoke_handler(generate_handler![get_snapshot, set_pinned, is_pinned])`. Build tests compare all three sets for equality and reject an extra command or permission.
- The three custom IPC commands are:
  1. `get_snapshot()` — no arguments; used for initial load and every refresh of the fixed startup root;
  2. `set_pinned(enabled: bool)` — calls Tauri `set_always_on_top`, then reads `is_always_on_top`, returning the actual boolean;
  3. `is_pinned()` — reads and returns the actual `is_always_on_top` boolean.
...
- Windows packaging explicitly sets `bundle.windows.webviewInstallMode` to `{ "type": "skip" }` and does not set `minimumWebview2Version`. The package uses the Owner-managed installed WebView2 runtime and never downloads, bootstraps, silently installs, or updates it. Packaging tests inspect configuration and artifacts for bootstrapper or update behavior.
...
- Load one Snapshot when the window starts. After each request settles, schedule the next request for two seconds later; never use overlapping interval requests. Maintain at most one request in flight.
...
- Open, Back, language, phase fold, scroll, Pin, refresh, and close never write or touch project records. Refresh reads only the fixed Rust-held root.
...
- Playwright drives the static Vite fixture at an exact `300 × 480` viewport. Capture exactly 28 golden targets: the main view plus each of the six protected reports (`7` views) × English and Chinese (`2`) × normal active motion and reduced motion (`2`).
...
- Screenshot comparison includes typography, wrapping, internal overflow, focus visibility, all four state treatments, six Open actions, protected notice, and absence of outer growth.
...
- On a real Windows display with WebView2, verify logical client size `300 × 480`, native decorations, non-resizable/maximize-disabled behavior, Open/Back, both languages, Pin on/off with actual host-state reads, two-second refresh, reduced motion, error/recovery, and clean close without callback or console errors.
- Security smoke attempts external/location navigation, link download, `window.open`, drag/drop of a file, every unregistered IPC name, and a path argument mutation. It asserts one window, no navigation/download/drop path, no command execution, no `core:default` or plugin permission, the exact CSP, and equal three-command build/capability/handler allowlists.
- Packaging inspection asserts `webviewInstallMode.type="skip"`, no WebView bootstrapper/minimum-version installer, no updater/plugin/artifact, no production source map, and no workspace/home absolute path in packaged binaries or resources.
- Hash every fixture before and after headless, DOM, visual, and packaged smoke runs; bytes and modification times must be unchanged.
...
The verified development baseline is Rust/Cargo 1.96.0, Node 24.13.1, npm 11.8.0, and WebView2 Runtime 150.0.4078.105. Implementation must not silently install or update system runtimes; dependency setup requires a separately authorized implementation action.

...
REQUIRED=[
'VERSION','README.md','README.zh-CN.md','CONSTITUTION.md','SPEC.md','CHANGELOG.md','MIGRATION-1.1.1-TO-2.0.0.md','MIGRATION-2.0.0-TO-2.1.0.md','MIGRATION-2.1.0-TO-2.2.0.md','MIGRATION-2.2.0-TO-2.2.1.md','MIGRATION-2.2.1-TO-2.2.2.md','MIGRATION-2.2.2-TO-2.2.3.md','MIGRATION-2.2.3-TO-2.3.0.md','MIGRATION-2.3.0-TO-2.4.0.md','MIGRATION-2.4.0-TO-2.4.1.md','MIGRATION-2.4.1-TO-2.5.0.md','MIGRATION-2.5.0-TO-2.5.1.md','MIGRATION-2.5.1-TO-2.5.2.md',
...
MARKERS=['Workflow capability end','UI product-surface end','Simulation World','Mandatory Calabash Upgrade','Product Baseline','Feature Slice','UI-locked Feature Integration','SLK / CLK / GLK','Independent layered Verification','Owner Acceptance','Delivery','INITIAL','PRODUCT_FORMATION','ENGINEERING_RUNS','DELIVERY_PREPARATION','Loop Owner Acceptance','Centralized Vulnerability Audit','Post-Security Owner Acceptance','Delivery Method Q&A','Existing engineering mode','Fixed mainline, proportional depth','authoritative durable project status','Execution Coverage Preflight','Owner gap']

def main():
    root=Path(sys.argv[1] if len(sys.argv)>1 else '.').resolve(); errors=[]
    for x in REQUIRED:
        if not (root/x).is_file(): errors.append('missing '+x)
    if (root/'VERSION').is_file() and (root/'VERSION').read_text().strip()!='2.5.2': errors.append('bad VERSION')

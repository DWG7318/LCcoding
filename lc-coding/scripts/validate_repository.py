...
REQUIRED=[
'VERSION','README.md','README.zh-CN.md','CONSTITUTION.md','SPEC.md','CHANGELOG.md','MIGRATION-1.1.1-TO-2.0.0.md',
...
'lc-coding/templates/DELIVERY-METHOD-QA.md','lc-coding/templates/DELIVERY-DECISION.json']
MARKERS=['Workflow capability end','UI product-surface end','Simulation World','Mandatory Calabash Upgrade','Product Baseline','Feature Slice','UI-locked Feature Integration','SLK / CLK / GLK','Independent layered Verification','Owner Acceptance','Delivery','INITIAL','PRODUCT_FORMATION','ENGINEERING_RUNS','DELIVERY_PREPARATION','Loop Owner Acceptance','Centralized Vulnerability Audit','Post-Security Owner Acceptance','Delivery Method Q&A']

def main():
    root=Path(sys.argv[1] if len(sys.argv)>1 else '.').resolve(); errors=[]
    for x in REQUIRED:
        if not (root/x).is_file(): errors.append('missing '+x)
    if (root/'VERSION').is_file() and (root/'VERSION').read_text().strip()!='2.0.0': errors.append('bad VERSION')

...
assert json.loads((root / "MANIFEST.json").read_text(encoding="utf-8"))["version"] == "2.1.0"
...
for relative in [
    ...
    assert json.loads((root / relative).read_text(encoding="utf-8"))["version"] == "2.1.0"

canonical = json.loads(
    (root / "lc-coding/templates/CANONICAL-MANIFEST.json").read_text(encoding="utf-8")
)
assert canonical["lccoding"]["version"] == "2.1.0"
for relative in ["README.md", "README.zh-CN.md", "CONSTITUTION.md", "SPEC.md", "lc-coding/SKILL.md"]:
    assert "2.1.0" in (root / relative).read_text(encoding="utf-8")
assert (root / "MIGRATION-2.0.0-TO-2.1.0.md").is_file()

print("PASS: LCCoding 2.1.0 version is consistent across release artifacts")

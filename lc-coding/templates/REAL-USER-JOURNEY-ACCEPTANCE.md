...
|---|---|---|---|---|---|---|---|---|---|---|---|
| JOURNEY-001 | <route-id> | DIRECT_PRODUCT / PERSONAL_AGENT / SERVICE_CENTER | <actor-id> / <actor-kind> | <action-id> / <resource-id> / <delegation-basis-id-or-NOT_APPLICABLE> | <real route entry> | <outcome understandable by the human beneficiary> | <comma-separated exact adopted IDs> | <acceptance-id> / sha256:<hash> / reviews/<receipt>.md | <Run-ID> / <D3-receipt-ID> | FINAL:<verification-id> / sha256:<hash> / <final-path>; D3:<D3-id> / sha256:<hash> / <D3-path> | REQUIRED |

Each Task5 citation resolves an authoritative `LOOP_OWNER_ACCEPTANCE_RECEIPT` named by the status acceptance index. Its exact candidate, route, Run, D3 receipt, canonical Run-start contract, and calling-phase return target must join the adopted route. The paired citations also resolve the actual `FINAL_FEATURE_VERIFICATION` and the existing Verification Receipt contract with `"layer": "D3"`; their candidate, route, authority, verdict, terminal evidence, receipt ID, coverage, and acceptance steps all join. The D3 artifact uses the canonical verification contract: D3 has reused or new evidence, may have structured or empty `repeated_checks`, and preserves the canonical independence checks. Task 6 does not define an alternate D3 shape. Matching or consistently renamed strings alone are not acceptance evidence.
...
|---|---|---|---|---|---|---|---|---|
| 1 | <candidate-id> / <sha256> | YES | <comma-separated attempted route IDs> | NONE, or <route-id> / DEPENDENCY_BLOCKED-or-GLOBAL_BLOCKED / <blocking-evidence-id> / <defect-id>; ... | <n> / <n> / <n> | <evidence IDs> | NONE / 40001,... | PASS / REWORK / BLOCKED |

The current round uses the current candidate. Historical rounds retain their own candidate identity. A failed or blocked round records first-hand evidence from each attempted entry through the last attempted or blocked step, identifies the resulting defects, and does not invent later unattempted steps. Every unattempted adopted route has one precise dependency/global-block disposition joined to real blocking evidence and a defect in that round; an independent route cannot silently disappear. A repaired candidate restarts the whole required route set from the beginning. Evidence bytes may not be copied across rounds under new IDs or paths.

## Route-faithful 4.0 evidence digests

For a visible direct-product step, use `SCREENSHOT` and keep the exact 3.0 `Action`, visible result, `Visible location`, and `Viewport` semantics. `Screenshot path` stays below `.lccoding/evidence/real-user-journey/`; the currently supported format is PNG, and the file must have a complete signature/chunk stream, valid chunk CRCs, IHDR/IDAT/IEND order, decodable compressed scanlines, nonzero dimensions, no trailing bytes, and its exact digest. Native evidence fields are `NOT_APPLICABLE`.

For a nonvisual Agent or assisted-service step, visible and screenshot fields are `NOT_APPLICABLE`; never fabricate a screenshot. `Native evidence path` resolves a strict, first-hand project evidence record with `record_role: REAL_USER_JOURNEY_EVIDENCE`, `evidence_schema_version: 4.0.0`, exact evidence/candidate/round/journey/route/step/actor/authority joins, and a kind-specific `event_or_result` object. Identity `result` is exactly `AUTHENTICATED / VERIFIED / MATCHED`; assisted-action `result` is exactly `PERFORMED / SUCCEEDED`; authorization `decision` is exactly `ALLOW / APPROVED`. User communication has exact direction `SERVICE_TO_HUMAN` and a separate exact `delivery_result` of `DELIVERED / ACKNOWLEDGED`. Combined or prefixed strings are not affirmative evidence. This is an offline project evidence contract, not a required external runtime or protocol.

Use only the kinds applicable to the adopted route and attempted step. Permitted Personal-Agent kinds are `HUMAN_GOAL_MESSAGE`, `AGENT_IDENTITY`, `REQUEST_MESSAGE`, `TASK_TRANSITION`, `AUTHORIZATION_DECISION`, `RESULT_ARTIFACT`, `PLATFORM_EFFECT`, `AGENT_RESPONSE`, `AUDIT_EVENT`, and `RESULT_DELIVERY`. A passing Personal-Agent route proves its request/message entry, Agent identity, affirmative route-bound authorization, platform effect, applicable audit facts, and result delivery. Permitted Service-Center kinds are `USER_REQUEST`, `SERVICE_ACTOR_IDENTITY`, `DELEGATION_BASIS`, `ASSISTED_ACTION`, `AUTHORIZATION_DECISION`, `PLATFORM_EFFECT`, `USER_COMMUNICATION`, `AUDIT_EVENT`, and `RESULT_DELIVERY`. A passing Service-Center route proves user request, service-actor identity, delegation basis, affirmative authorization, assisted action, platform effect, user communication, applicable audit facts, and result delivery. These are route-derived requirements, not a universal all-kinds matrix. Every passing route ends in first-hand evidence of the adopted human-observable outcome.

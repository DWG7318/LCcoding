...
Each Task5 citation resolves an authoritative `LOOP_OWNER_ACCEPTANCE_RECEIPT` named by the status acceptance index. Its exact candidate, route, Run, D3 receipt, canonical Run-start contract, and calling-phase return target must join the adopted route. The paired citations also resolve the actual `FINAL_FEATURE_VERIFICATION` and the existing Verification Receipt contract with `"layer": "D3"`; their candidate, route, authority, verdict, terminal evidence, receipt ID, coverage, and acceptance steps all join. Matching or consistently renamed strings alone are not acceptance evidence.

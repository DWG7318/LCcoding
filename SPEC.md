# LCCoding Standard Specification 4.0.1
...
The LCCoding entry path begins with `LCCODING_APPLICABILITY_ASSESSMENT` at the Initial boundary. The assessment judges whether the full LCCoding lifecycle fits the proposed engineering object; it does not judge the project's value or technical merit. Its result is exactly `WHOLE_PRODUCT_FIT`, `BOUNDED_PRODUCT_FIT`, or `OTHER_METHOD_RECOMMENDED`. `WHOLE_PRODUCT_FIT` and `BOUNDED_PRODUCT_FIT` admit the assessed whole or complete bounded product scope into `PROPOSAL_READINESS`, `PRODUCT_SERVICE_STRATEGY`, and `PROJECT_INITIALIZATION`, and then the unchanged `INITIAL_READY` gate. `OTHER_METHOD_RECOMMENDED` is a terminal assessment disposition for LCCoding: it has no next lifecycle node and does not enter Readiness, Strategy, Initialization, or later LCCoding work. It rejects only use of the full method for the assessed scope and does not reject or stop the underlying project. When facts are insufficient, no applicability result is issued; the disposition is `PROPOSAL_INCOMPLETE`, and assessment resumes when the missing facts are supplied rather than silently becoming a negative judgment.
...
LCCoding, Calabash, SLK, CLK, GLK, and LCCoding-shipped contracts, templates, scripts, references, and BI are native. Ponytail is the only named curated external Skill in this release. Before its first load, installation, invocation, Agent injection, or governing use in a project, AI explains its source, recommendation, proposed scope and mode, benefits, and material trade-offs and obtains one affirmative Owner authorization. Read-only examination and the inquiry itself are not activation; rejection or non-adoption continues through native LCCoding and creates no new stop condition. Authorization remains limited to the recorded project, identity, scope, and mode and grants no additional product, repository, system, release, or delivery authority. Availability or global installation is not project authorization, and any future named external Skill requires separate later review rather than incidental admission.
...
`INITIAL` admits only `WHOLE_PRODUCT_FIT` or `BOUNDED_PRODUCT_FIT`. Its successful path contains, in order, LCCoding Applicability Assessment, Proposal Readiness, Product Service Strategy, and Project Initialization, and exits through the existing `INITIAL_READY` gate. These are fine milestones, not phases or gates. `OTHER_METHOD_RECOMMENDED` ends the LCCoding assessment before the remaining fine milestones; `PROPOSAL_INCOMPLETE` pauses admission without manufacturing an applicability outcome. Proposal Readiness reads supplied material once, builds one completeness view, asks only blocking or materially risky unresolved questions, offers a recommended answer with concise alternatives, persists each answer, and returns only `PROPOSAL_READY` or `PROPOSAL_INCOMPLETE`.
...
`PRODUCT_FORMATION` starts after `INITIAL_READY` and contains, in order of dependency, Calabash Draft, the Calabash-owned Service Route Map, the route-neutral `WORKFLOW_ROUTE_SURFACES_SIMULATION` milestone, Mandatory Calabash Upgrade, and Product Baseline. The route-neutral milestone builds a Simulation-first foundation, real runnable Workflow, and only the direct-product, Agent-service, assisted-service, and human-observable surfaces or adapters selected for required delivered journeys; graphical UI is required only where a promised route needs it, and artificial UI is forbidden:
...
`WORKFLOW_UI_SIMULATION` remains a read-only compatibility name for the 3.0 direct-product formation shape. It resolves to `WORKFLOW_ROUTE_SURFACES_SIMULATION` and cannot make a UI product-surface end mandatory for a route that does not promise one.
...
Before actual Workflow or route-specific surface or adapter construction begins, Product Formation requires at least one minimal, real, runnable, versioned Simulation World foundation. It is a starting Product Simulation World, not a complete or frozen Simulation. Workflow, applicable direct-product, Agent-service, assisted-service and human-outcome surfaces, UI where promised, and Simulation are built separately during Product Formation. Only after that foundation exists may Workflow and the applicable route ends advance independently; each required end must produce real, runnable, inspectable results. Early Product Formation does not require Workflow and those ends to be connected or jointly integrated. Cross-layer Workflow-to-route connection and end-to-end proof remain responsibilities of Feature Slice and route-bound Integration; the one-way UI lock still applies wherever UI exists. Simulation remains `VERSIONED_MUTABLE` and gains scenarios and fidelity through versioned deltas.
...
The adopted Service Route Map selects applicable product surfaces and adapters per delivered journey. A direct-product surface, Agent-service surface, assisted-service surface, and human-observable outcome surface are built only when required by a promised route. UI covers the human-visible surfaces that are actually part of those routes; LCCoding must not invent graphical UI for an Agent-first, CLI, API-first, or assisted journey that does not promise one. Applicable surfaces or adapters, Workflow, and Simulation remain separately runnable and inspectable during formation while sharing product meaning and scenario IDs. Their synchronization is not early cross-layer integration.
...
Product Baseline Handoff freezes one total project repository at an exact project commit (full resolvable SHA) and locks every realized route-specific surface or adapter, UI where applicable, Workflow, and Simulation subtree by ID/name, safe path, `MAJOR.MINOR.PATCH` component version, and deterministic content hash. Maps and Handoff must agree on identities, Workflow classification and API/MCP evidence, relations, and Owner confirmation.

When multiple subtrees exist, the Owner confirms one Primary product mainline linking at least one peer Product Simulation, one CORE Workflow, the route-specific surface or adapter required by that mainline, and a human-observable outcome. A graphical UI participates only when promised by the route. The mainline selects first proving/construction priority only; every other CORE remains mandatory. A controlled Calabash upgrade may reconfirm the relation.
...
LCCoding chooses the lightest truthful topology from current applicable method information before locking one exact identity: SLK for one serial Loop path, CLK for multiple independently progressing Chains with contract-governed fusion, GLK for a dependency DAG converging through its final Fusion GO, or another registered execution method whose compatible contract fits the bounded work. One method owns a Run. Methods own their internal GO/CELL/Chain/graph/task/retry topology; LCCoding does not copy it or silently follow later method releases after the identity is locked.
